...
// Get activity heatmap - traffic by hour of day and day of week
export const getActivityHeatmap = async (limit: number = 2000): Promise<{
  hourlyHeatmap: Array<{ hour: number; dayOfWeek: number; traffic: number; uniqueUsers: number }>;
  peakHours: Array<{ hour: number; traffic: number }>;
  peakDays: Array<{ dayOfWeek: string; traffic: number }>;
}> => {
  try {
    const ADMIN_EMAIL = 'nafisabdullah424@gmail.com';
    const snapshot = await db.collection('globalPageNavigation')
    ...
    // Map to track heatmap data
    const heatmapData = new Map<string, { traffic: number; users: Set<string> }>();
    const hourCounts = new Map<number, { traffic: number; users: Set<string> }>();
    const dayCounts = new Map<number, { traffic: number; users: Set<string> }>();
    ...
    snapshot.docs.forEach(doc => {
      ...
      // Only count actual page views, not exits
      ...
      // Update heatmap
      const existing = heatmapData.get(key) || { traffic: 0, users: new Set() };
      existing.traffic++;
      existing.users.add(data.userId);
      heatmapData.set(key, existing);

      // Update hour counts
      const hourExisting = hourCounts.get(hour) || { traffic: 0, users: new Set() };
      hourExisting.traffic++;
      hourExisting.users.add(data.userId);
      hourCounts.set(hour, hourExisting);

      // Update day counts
      const dayExisting = dayCounts.get(dayOfWeek) || { traffic: 0, users: new Set() };
      dayExisting.traffic++;
      dayExisting.users.add(data.userId);
      dayCounts.set(dayOfWeek, dayExisting);
    });
