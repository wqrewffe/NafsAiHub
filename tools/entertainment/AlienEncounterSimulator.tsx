...
export const renderAlienEncounterSimulatorOutput = (output: AlienReportOutput | string) => {
    ...
    return (
        <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 font-mono text-green-300">
            <h2 className="text-xl font-bold text-center border-b-2 border-green-300/50 pb-2">[ GALACTIC FEDERATION - CLASSIFIED REPORT ]</h2>
            <div className="mt-4 space-y-3 text-sm">
                <p><span className="text-green-500">{`>`} SUBJECT:</span> {data.objectName} (Human designation)</p>
                <p><span className="text-green-500">{`>`} XENO-DESIGNATION:</span> {data.alienDesignation}</p>
                <p><span className="text-green-500">{`>`} ANALYSIS:</span></p>
                <p className="pl-4 whitespace-pre-wrap">{data.analysis}</p>
                <p><span className="text-green-500">{`>`} RECOMMENDATION TO HIGH COUNCIL:</span></p>
