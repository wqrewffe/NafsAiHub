  ...
  useEffect(() => {
    ...
    const fetchUserProgress = async () => {
      ...
      if (doc) {
        ...
        // Check for completed quests and trigger celebrations
        const completedQuests = doc.quests.filter(quest => quest.completed);
        if (completedQuests.length > 0) {
          ...
          completedQuests.forEach((quest, index) => {
            setTimeout(() => {
              setXpAnimation({ amount: quest.xpReward, visible: true });
              setTimeout(() => setXpAnimation(prev => ({ ...prev, visible: false })), 1500);
            }, index * 2000);
          });
        }
      }
